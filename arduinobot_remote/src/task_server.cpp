...
//THIS IS THE TASK SERVER FOR ATOMIC ACTIONS
...
//THIS IS THE RESPONSE FOR WHEN A VALID GOAL IS RECEIVED
private:
  ...
  {
    // THIS IS THE PRINTING OF THE REQUESTED OBJECTIVE
    RCLCPP_INFO(get_logger(), "Received goal request with id %d", goal->task_number);
    (void)uuid;
    return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
  }
//THIS IS THE REQUEST TO CANCEL A GOAL FOR THE ACTION SERVER
  ...
  void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<arduinobot_msgs::action::ArduinobotTask>> goal_handle)
  {
    ...
    // THESE COMMANDS USE MOVEIT API TO SEND A INSTRUCTION TO THE GROUPS THAT EXIST IN THE ROBOT
    auto arm_move_group = moveit::planning_interface::MoveGroupInterface(shared_from_this(), "arm");
    auto gripper_move_group = moveit::planning_interface::MoveGroupInterface(shared_from_this(), "gripper");
    ...
    // THESE CONDITIONS CONTAIN THE OBJECTIVE 
    // THIS ONE IS TO GO TO ACTIVATE POSITION
    if (goal_handle->get_goal()->task_number == 0)
    {
      arm_joint_goal = {0.0, 0.0, 0.0};
      gripper_joint_goal = {-0.7, 0.7};
    }
    // THIS CONDITION IS TO GO TO GRIP GOAL POSE
    else if (goal_handle->get_goal()->task_number == 1)
    {
      arm_joint_goal = {-1.14, -0.6, -0.07};
      gripper_joint_goal = {0.0, 0.0};
    }
    // THIS ONE IS TO GOT TO SLEEP POSE
...
//THIS INDICATES WHEN A GOAL POSITION IS OUT OF BOUNDS FOR THE TASK SERVER
...
//THIS HANDLES THE VALID RESPONSE WHEN THE TRAJECTORY IS VALID
    ...
    //THIS HANDLES INVALID REQUESTS WHERE THE PLANNING FAILS
    else
    {
